...
def play_memory(file_path):
    """Opens the memory file in the default player."""
...
def run_guided():
    ...
    if music_input:
        type_writer(f"    >> Soundtrack set: {os.path.basename(music_input)}")
    ...
    # Create placeholder if API isn't running, so logic works
